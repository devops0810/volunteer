...
trait HelperTrait {
    ...
    public function canAddUsers(){
        ...
        if($totalUsers>=USER_LIMIT && USER_LIMIT > 0){
            $this->errorMessage(request(),__('saas.user-error'));
            return false;
        }
        else{
            return true;
    ...
    public function canAddDepartments(){
        $totalDepartments = Department::count();
        if($totalDepartments >= DEPARTMENT_LIMIT && DEPARTMENT_LIMIT > 0){
            $this->errorMessage(request(),__('saas.dept-error'));
            return false;
        }
        else{
            return true;
